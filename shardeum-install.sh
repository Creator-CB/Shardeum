...
if [ -d "$NODEHOME" ]; then
  if [ "$NODEHOME" != "$(pwd)" ]; then
    ...
    echo "Невозможно удалить текущую рабочую версию. Пожалуйста, перейдите в другую директорию и повторите попытку."
  fi
fi
...
echo "Начальное изображение. Это может занять некоторое время..."
